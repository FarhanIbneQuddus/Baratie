      ...
      <div className='max-w-6xl mx-auto pl-20 pt-20'>
        ...
        <div className='mb-8 flex flex-warp gap-6'>
          {dataArray.map((item, index)=>(
            <div key={index} className='flex flex-col items-center justify-center gap-1 border h-20 w-40 border-gray-200 bg-gradient-to-r from-indigo-200 to-blue-100 shadow rounded-md'>
        ...
        {/* Tabs */}
        <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-gradient-to-r from-indigo-200 to-blue-100 shadow-sm'>
          {
            dataArray.map((tab)=>(
              <button onClick={()=> setCurrentTab(tab.label)} key={tab.label} className={`cursor-pointer flex items-center px-3 py-1 text-sm rounded-md transition-colors ${currentTab === tab.label ? 'bg-gradient-to-r from-indigo-400 to-blue-300 font-medium text-black' : 'text-gray-500 hover:text-black'}`}>
        ...
        <div className='flex flex-wrap gap-6 mt-6'>
          {dataArray.find((item)=>item.label === currentTab).value.map((user)=>(
            <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-gradient-to-r from-indigo-100 to-blue-300 shadow rounded-mb'>
